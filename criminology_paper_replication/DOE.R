...
explore_kpclr_obj = explore_kpclr_models(Xd, y, kernel_list = kernel_list, fn_cost = 10, fp_cost = 1, num_cores = 3)
#fn_max_cost = 6, fn_min_cost = 4, fp_max_cost = 1, fp_min_cost = 1
windows()
plot(explore_kpclr_obj, quantile_cwe_to_display = 0.99, tile_cols = 4, kernels_to_plot = 1 : 3, ylim = c(0,.2))#, min_fn_fp_ratio = 1/5.5, max_fn_fp_ratio = 1/4.5)
...
kpca_obj = build_kpca_object(explore_kpclr_obj$X_train, kernel_type = "anova", c(1,3))
weights = weights_for_kpclr(explore_kpclr_obj$y_train, fn_to_fp_ratio = 10)
mod = kpclr(kpca_obj, explore_kpclr_obj$y_train, frac_var = .85, weights = weights)
preds = predict(mod, new_data = explore_kpclr_obj$X_test, num_cores = 4)
