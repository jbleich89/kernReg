...
##For Richard's stuff
weights = weights_for_kpclr(explore_kpclr_obj$y_train, 1/2)
kpca_obj = build_kpca_object(X=explore_kpclr_obj$X_train, kernel_type =  "anova", params = c(100,3))
mod = kpclr(kpca_object = kpca_obj, y = explore_kpclr_obj$y_train ,frac_var = .75, weights = weights)
phat_val = predict(mod, new_data = explore_kpclr_obj$X_train, num_cores = 3)
windows()
par(mar = c(4,4,0,1))
hist(phat_val, br = 50, xlab = "Predicted Probability of an FTA-Validation", main = "")
phat_test = predict(mod, new_data = explore_kpclr_obj$X_test, num_cores = 3)
hist(phat_test, br = 50, xlab = "Predicted Probability of an FTA-Test", main = "")
table(explore_kpclr_obj$y_test, phat_test > .5)


### Figure 7
explore_kpclr_obj = explore_kpclr_models(X, y, fp_cost = 2, kernel_list = kernel_list, rho_seq = seq(.6,.95, .05), num_cores = 3)
...
plot(explore_kpclr_obj_win, tile_cols = 3, min_fn_fp_ratio = 1, max_fn_fp_ratio = 4)
